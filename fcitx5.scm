  ...
  (package
    ...
    (source (origin
		    ...
		    (commit version)))
              (sha256
               (base32
                "1rg86jvlh7l8njzvb9d6gnqhnhnhkm786xf4nsxys669dk44xbrp"))))
    (build-system cmake-build-system)
    (arguments
     '(#:tests? #f ;no tests
       #:configure-flags (list (string-append "-DSKK_DEFAULT_PATH=" (assoc-ref %build-inputs "skk-jisyo") "/share/skk/SKK-JISYO.L"))
       ))
    (inputs (list fcitx5 libskk gobject-introspection qtbase-5 fcitx5-qt))
    (native-inputs (list gettext-minimal extra-cmake-modules pkg-config skk-jisyo))
    (home-page "https://github.com/fcitx/fcitx5-skk")
    (synopsis "fcitx-skk is an input method engine for Fcitx, which uses libskk as its backend.")
    (description
